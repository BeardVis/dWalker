...
function dWalker(args) {
    ...
    window.onmousemove = e => { e = event || window.event; items.forEach(item => item.SetNewPosition(new Point(e.clientX, e.clientY))); items.forEach(item => SetLineNewPosition(item)); };
}
...
class Line {
    ...
    Update(from, to) {
        ...
        let cPoint = new Point(from.X, to.Y);
        let a = getVectorLength(from, cPoint);
        let b = getVectorLength(to, cPoint);
        let k = 0, tmp;
        ...
        let res = (Math.atan2(a, b) * 180 / Math.PI) + k;
        this.hr.style.top = `${from.Y}px`;
        this.hr.style.left = `${from.X}px`;
        this.hr.style.width = `${getVectorLength(from, to)}px`;
        this.hr.style.transform = `rotate(${res * -1}deg)`;
    }
